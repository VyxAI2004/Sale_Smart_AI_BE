...
class ProductTrustScoreService(
    BaseService[
        ProductTrustScore,
        ProductTrustScoreCreate,
        ProductTrustScoreUpdate,
        ProductTrustScoreRepository,
    ]
):
    ...
    def get_top_trusted(
        self, project_id: Optional[UUID] = None, limit: int = 10
    ) -> List[ProductTrustScore]:
        return self.repository.get_top_trusted(project_id=project_id, limit=limit)
    ...
    def calculate_trust_score(self, product_id: UUID) -> Optional[ProductTrustScore]:
        ...
        verification_factor = self._calculate_verification_factor(
            review_stats["verified_purchases"], total_reviews
        )
        ...
        trust_score_data = ProductTrustScoreCreate(
            ...
            average_sentiment_score=Decimal(
                str(analysis_stats.get("average_sentiment_score", 0.5))
            ),
            calculation_metadata={
                "formula_version": "2.0-with-sentiment",
                "weights": {
                    "sentiment_factor": self.SENTIMENT_WEIGHT,
                    "spam_factor": self.SPAM_WEIGHT,
                    "volume_factor": self.VOLUME_WEIGHT,
    ...
    def _calculate_sentiment_factor(self, analysis_stats: dict) -> float:
        return float(analysis_stats.get("average_sentiment_score", 0.5))

    def _calculate_spam_factor(self, analysis_stats: dict) -> float:
        if analysis_stats.get("total_analyzed", 0) == 0:
            return 1.0
        return 1.0 - float(analysis_stats.get("average_spam_score", 0.0))

    def _calculate_volume_factor(self, total_reviews: int) -> float:
        if total_reviews == 0:
            return 0.0
        return min(math.log(total_reviews + 1) / math.log(1001), 1.0)

    def _calculate_verification_factor(
        self, verified_count: int, total_reviews: int
    ) -> float:
        if total_reviews == 0:
            return 0.0
        return verified_count / total_reviews

    def _update_product_trust_score(
        self, product_id: UUID, trust_score: float
    ) -> None:
        product = self.product_repo.get(product_id)
        if product:
            self.product_repo.update(
                db_obj=product,
                obj_in=ProductUpdate(trust_score=round(trust_score, 2)),
            )

    def get_trust_score_detail(
        self, product_id: UUID
    ) -> Optional[TrustScoreDetailResponse]:
        ...
        if "sentiment_factor" in component_scores:
            ...
            avg_sentiment = float(trust_score.average_sentiment_score)
        ...
        if "spam_factor" in component_scores:
            ...
            avg_spam_score = analysis_stats.get("average_spam_score", 0.0)
            breakdown["spam"] = TrustScoreBreakdown(
                ...
                details={
                    ...
                    "not_spam": trust_score.total_reviews
                    - trust_score.spam_reviews_count,
                    "spam_percentage": float(trust_score.spam_percentage),
                    "average_spam_score": float(avg_spam_score),
                    "bias": 1.0 - float(avg_spam_score),
                    "raw_bias": 1.0 - float(avg_spam_score) * 2,
                },
            )
        ...
        if "verification_factor" in component_scores:
            ...
            verification_rate = (
                trust_score.verified_reviews_count
                / trust_score.total_reviews
                * 100
                if trust_score.total_reviews > 0
                else 0
            )
